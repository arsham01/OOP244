...
namespace sdds{
  ...
  bool Robot::isEmpty() const{
    return strcmp(r_nickname, "") == 0 && r_durability == 0 && r_arms < 1 && r_legs < 1;
  }
  ...
  {
    r_arms += robot.r_arms.getForce();
    r_legs.setSpeed(robot.r_legs.getSpeed());
    r_durability = robot.r_durability;
    strcpy(r_nickname, robot.r_nickname);
    return *this;
  }
